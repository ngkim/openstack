...
ctrl_glance_api_registry_configure() {
    ...
    cp ${GLANCE_API_CONF} ${GLANCE_API_CONF}_$(date +"%Y%m%d-%T").bak
...
flavor = keystone" | tee -a ${GLANCE_API_CONF}    
    ...
    cp ${GLANCE_REGISTRY_CONF} ${GLANCE_REGISTRY_CONF}_$(date +"%Y%m%d-%T").bak
    sed -i 's/^#known_stores.*/known_stores = glance.store.filesystem.Store,\
                   glance.store.http.Store,\
...
ctrl_glance_demo_image_create() {
    ...
    then
        ...
        wget --quiet https://launchpad.net/cirros/trunk/0.3.0/+download/cirros-0.3.0-x86_64-disk.img -O ./${CIRROS_IMAGE}
    fi
