...
ctrl_horizon_configure() {
    ...
    #Set default role
    sudo sed -i "s/OPENSTACK_HOST = \"127.0.0.1\"/OPENSTACK_HOST = \"${CTRL_API_IP}\"/g" $HORIZON_CONF 
    
    #Move /horizon to /
    sudo sed -i "s@LOGIN_URL.*@LOGIN_URL='/auth/login/'@g" $HORIZON_CONF 
    sudo sed -i "s@LOGOUT_URL.*@LOGOUT_URL='/auth/logout/'@g" $HORIZON_CONF 
    sudo sed -i "s@LOGIN_REDIRECT_URL.*@LOGIN_REDIRECT_URL='/'@g" $HORIZON_CONF 
