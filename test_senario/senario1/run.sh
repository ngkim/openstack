...
source ./50make_vm_for_test.sh    
    make_user_utm_vm    ${customer}utm    $UTM_IMAGE    global_mgmt_net $GREEN_NET  $ORANGE_NET $RED_NET seocho.seoul.zo.kt havana 53utm_bootstrap.sh
    echo
    make_user_green_vm  ${customer}client $CLIENT_IMAGE global_mgmt_net $GREEN_NET  seocho.seoul.zo.kt cnode02 51green_bootstrap.sh
    echo
    make_user_orange_vm ${customer}server $SERVER_IMAGE global_mgmt_net $ORANGE_NET seocho.seoul.zo.kt cnode02 52orange_bootstrap.sh
    echo
