	...
	*/
		...
		(SELECT
				GROUP_CONCAT( CONCAT('host total_vcpu(x5): ',ncn.vcpus*5
					...
					,', total_mm(MB): ',ncn.memory_mb
					,', used_mm(MB): ',ncn.memory_mb_used
					,', percent_mm(%): ',FORMAT((ncn.memory_mb_used/ncn.memory_mb)*100,1)
					) SEPARATOR ' || ')
			 FROM
				nova.compute_nodes 		AS ncn
			WHERE 
				ni.host = ncn.hypervisor_hostname) AS vm_host_infos,
	...
	-- project 관련 network quota & quota usages
		(SELECT
				GROUP_CONCAT( CONCAT(qq.resource,' -> ', qq.hard_limit) SEPARATOR ', ')
			 FROM
				neutron.quotas AS qq
			 WHERE
				ni.project_id = qq.project_id ) AS vm_project_network_quotas,

		(SELECT
				GROUP_CONCAT( CONCAT(qqu.resource,' -> ', qqu.in_use) SEPARATOR ', ')
			 FROM
				neutron.quota_usages AS qqu
			 WHERE
	...
	*/
	...
	-- vm's volume info
		...
		(SELECT
		 ...
		 WHERE
			vol.deleted_at IS NULL AND
			ni.uuid = vol.instance_uuid) AS "vm_vol_size_sum(GB)",
