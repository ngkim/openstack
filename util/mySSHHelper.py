...
class myRSSH():
    ...
    def delegate (self, pw, mode, cmd):
        ...
        reach_eof = False
        try:            
            i = child.expect([pexpect.TIMEOUT, ssh_newkey, '[pP]assword: ', pexpect.EOF], timeout=self.timeout)            
            if i == 0: # Timeout
...
def test_rssh_batch_command(rconsole, tag, cmd):
    # using doRemoteCommand example
    print "#"*80        
    print "### using doRemoteCommand example"
    result = rconsole.doRemoteCommand (tag, cmd)
    print result
    print "#"*80

def test_rssh_interactive_command(rconsole, tag):
    # using getRemoteShell example        
    print "#"*80        
    print "### using getRemoteShell example"
    ctrl = rconsole.getRemoteShell(tag)
...
if __name__ == '__main__':
    ...
    test_rssh_batch_command(rssh, 'ctrl', cmd)      
    #test_rssh_interactive_command(rssh, 'ctrl') 